...
Lean mode uses a 512 MiB OpenSearch heap and disables only `docspace-ai-worker`, `docspace-mcp` and `docspace-telegram`. The browser-facing `docspace-ai` service and backup services remain enabled to avoid 502 responses from normal UI/API routes.
...
- can install a persistent lean-mode watcher that re-disables selected optional services after dpkg/package changes;
